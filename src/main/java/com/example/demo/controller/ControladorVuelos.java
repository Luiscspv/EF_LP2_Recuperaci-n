...
public class ControladorVuelos {
	...
	@Autowired
	private VentaDescripcionService detalleVentaService;
    ...
    public String registrarBoletos(
        ...
        List<VentaDescripcion> lista_detalles = (List<VentaDescripcion>) session.getAttribute("lista_detalles");
        ...
        VentaDescripcion detalle_venta = VentaDescripcion.builder()
                .venta(null)
        ...
        Double monto_total = lista_detalles.stream().mapToDouble(VentaDescripcion::getSubTotal).sum();
	...
	@GetMapping("/CompradeBoleto")
	public String compradeBoleto(Model model, HttpSession session) {
	    @SuppressWarnings("unchecked")
		List<VentaDescripcion> lista_detalles = (List<VentaDescripcion>) session.getAttribute("lista_detalles");
